...
class DrawChart {
  ...
  setYRange() {
    ...
    if (
      this.ChartWindow.lockUpdatePriceRange.peek() &&
      this.yAxisRange.peek().maxPrice > result.maxPrice &&
      this.yAxisRange.peek().minPrice < result.minPrice
    )
      return;
    if (
      result &&
  ...
  setYAxisConfig() {
    const priceDiff =
      this.yAxisRange.peek().maxPrice - this.yAxisRange.peek().minPrice;
    let colDiff = this.yAxisConfig.peek().colDiff;
    const priceMap = Object.keys(priceToColMap);
    priceMap.forEach((price, i) => {
      if (
        parseInt(price) > priceDiff &&
        priceDiff > parseInt(priceMap[i - 1])
      ) {
