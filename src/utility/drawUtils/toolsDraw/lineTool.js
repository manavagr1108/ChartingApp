...
export const drawHorizontalRayUsingPoints = (state, canvas, points, lineSelected = false, ctx1 = null) => {
    const [startCoords] = getCoordsArray(state, points);
    const { chartCanvasSize } = state;
    drawTrendLineUsingPoints(state, canvas, [{ x: startCoords.x, y: startCoords.y }, { x: chartCanvasSize.peek().width, y: startCoords.y }], false, ctx1);
    if (lineSelected && ctx1 !== null) {
        ctx1.fillStyle = "White";
        ctx1.strokeStyle = "blue";
        ctx1.beginPath();
